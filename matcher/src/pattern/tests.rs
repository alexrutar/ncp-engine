...
#[test]
fn multi_codepoint_canonically_equivalent_needles() {
    let nfc = Atom::new(
        "ấ",
        CaseMatching::Respect,
        Normalization::Smart,
    ...
    assert_eq!(nfc.needle, nfd.needle);
    assert_eq!(nfd.needle.to_string(), "ấ");
    assert!(!nfd.normalize);

    let nfc_haystack = Utf32String::from("ấ");
    let nfd_haystack = Utf32String::from("a\u{0302}\u{0301}");
    let plain_haystack = Utf32String::from("a");
    let mut matcher = Matcher::default();
